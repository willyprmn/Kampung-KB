...
use Cache;
use App\Models\Portal\Statistik\KkbpkJumlahPesertaPoktan;
...
class KkbpkJumlahPesertaPoktanDataTable extends CachedDataTable
{
     ...
     *
     * @param \App\Models\Portal\Statistik\KkbpkJumlahPesertaPoktan $model
    ...
    public function query()
    {
        ...
        $request['statistic'] = 'kkbpk-jumlah-peserta-poktan';
        $key = md5(json_encode($request));

        #set 1 day expired cache
        $data = Cache::remember($key, config('kpkb.statistik.cache.lifetime'), function (){

            $sql = file_get_contents(base_path('database/sql/statistik/kkbpk/jumlah-peserta-poktan.sql'));
    ...
    {
        return 'kkbpk-jumlah-peserta-poktan-table_' . date('YmdHis');
    }
}
