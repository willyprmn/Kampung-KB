...
namespace App\DataTables\Portal\Statistik\Intervensi\FungsiKeluarga;
...
class FungsiKeluargaDataTable extends CachedDataTable
{
    ...
    public function query()
    {
        ...
        $request['statistic'] = 'inpres-data-administrasi';
        $key = md5(json_encode($request));

        #set 1 day expired cache
        $data = Cache::remember($key, config('kpkb.statistik.cache.lifetime'), function (){

            $sql = file_get_contents(base_path('database/sql/statistik/intervensi/fungsi-keluarga/fungsi-keluarga.sql'));
            #alias table master
            $provinsi = "d";
            $kabupaten = "e";
            $kecamatan = "f";
    ...
    protected function getColumns()
    {
        return [
            ...
            'pembinaan_lingkungan' => [
                'title' => 'Pembinaan Lingkungan',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'lainnya' => [
            ...
            'kegiatan_1' => [
                'title' => 'Melaksanakan 1-3 Kegiatan',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'kegiatan_2' => [
                'title' => 'Melaksanakan 4-6 Kegiatan',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'kegiatan_3' => [
                'title' => 'Melaksanakan 7-9 Kegiatan',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'belum_isi' => [
    ...
    {
        return 'statistik-intervensi-8 fungsi keluarga-' . date('YmdHis');
    }
}
