...
class InpresPenataanLingkunganKeluargaDataTable extends CachedDataTable
{
    ...
    public function query()
    {
        ...
        $request['statistic'] = 'inpres-penataan-lingkungan-keluarga';
        $key = md5(json_encode($request));

        #set 1 day expired cache
        $data = Cache::remember($key, config('kpkb.statistik.cache.lifetime'), function (){

            $sql = file_get_contents(base_path('database/sql/statistik/intervensi/inpres-kegiatan/table8_penataan_lingkungan_keluarga.sql'));
            #alias table master
            $provinsi = "d";
            $kabupaten = "e";
            $kecamatan = "f";
    ...
    protected function getColumns()
    {
        return [
            ...
            'stbm' => [
                'title' => 'Pemicuan Sanitasi Total Berbasis Masyarakat (STBM)',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'akses_perumahan' => [
                'title' => 'Penyediaan Akses Perumahan dan infrastruktur permukiman yang layak, aman dan terjangkau',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'pengendalian_vektor' => [
                'title' => 'Pengendalian vektor dan binatang pembawa penyakit',
                'orderable' => false,
                'className' => 'colnum',
            ],
            'kegiatan_1' => [
