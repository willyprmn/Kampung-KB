...
(
    ...
    left join (
        select %2$s.id, %2$s.name,
            sum(case when b.plkb_pendamping_flag is true then 1 else 0 end) ada,
            sum(case when (b.plkb_pendamping_flag is null or b.plkb_pendamping_flag is false) and b.plkb_pengarah_id is not null then 1 else 0 end) tidak_ada,
            sum(case when
					(b.plkb_pendamping_flag is null or b.plkb_pendamping_flag is false) and b.plkb_pengarah_id is null then 1
				else 0
			end) belum_ada
