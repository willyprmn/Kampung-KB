select a.id regional_id, a.name regional, b.*,
    case when(total > 0) then ROUND( ((dasar / total::float)*100)::decimal, 2 ) else 0 end dasar_persen,
    case when(total > 0) then ROUND( ((berkembang / total::float)*100)::decimal, 2 ) else 0 end berkembang_persen,
    case when(total > 0) then ROUND( ((mandiri / total::float)*100)::decimal, 2 ) else 0 end mandiri_persen,
    case when(total > 0) then ROUND( ((berkelanjutan / total::float)*100)::decimal, 2 ) else 0 end berkelanjutan_persen

from %1$s a
left join (
	select %2$s.id, %2$s.name regional,
		sum(dasar) dasar,
		sum(berkembang) berkembang,
		sum(mandiri) mandiri,
		sum(berkelanjutan) berkelanjutan,
		count(1) total
	from (
		select *,
