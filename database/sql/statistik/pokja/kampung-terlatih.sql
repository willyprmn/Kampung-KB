...
(
    ...
    left join (
        select %2$s.id, %2$s.name,
            sum(case when b.pokja_pengurusan_flag is true and pokja_pelatihan_flag is true then 1 else 0 end) ada,
            sum(case when (b.pokja_pengurusan_flag is false or b.pokja_pengurusan_flag is true) and pokja_pelatihan_flag is false then 1 else 0 end) tidak_ada,
            sum(case when b.pokja_pengurusan_flag is null or (b.pokja_pengurusan_flag is true and pokja_pelatihan_flag is null)  then 1 else 0 end) belum_ada
